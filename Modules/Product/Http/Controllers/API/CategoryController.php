...
use App\Http\Resources\Api\v1\Brand\ColorResource;
use Modules\Product\Transformers\CategoryResource;
use Modules\Product\Repositories\AttributeRepository;
use App\Http\Resources\Api\v1\Category\CategoryAttributeResource;
    ...
    public function index(){
        ...
        if(count($categories) > 0){
            ...
            return response()->json([
                'message' => trans('app.category not found')
            ],404);
        }
    ...
    *        }
    ...
    public function show($id){
        ...
        if($category){
            $getColor = null;
            if(isset($color)){
                $getColor = new ColorResource($color);
            }
            return response()->json([
                'data' => new CategoryResource($category),
                'attributes' => count($attributes) > 0 ? CategoryAttributeResource::collection($attributes):null,
                'color' => $getColor,
                'brands' => count($brands) > 0 ?  BrandResource::collection($brands):null,
