                ...
                <div class="dropdown CRM_dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">
                        ...
                        @if(permissionCheck('page_builder.pages.update'))
                            <a href="#" class="dropdown-item edit_row" data-url="{{ route('page_builder.pages.edit',$row->id) }}">{{ __('common.edit') }}</a>
                        @endif
                        @if(permissionCheck('page_builder.pages.destroy'))
                            @if($row->slug == 'about-us' || $row->slug == 'affiliate')
                            @else
                                <a href="#" class="dropdown-item delete_row" data-url="{{ route('page_builder.pages.deleteModal',$row->id) }}" type="button">{{ __('common.delete') }}</a>
                            @endif
                        @endif
                    </div>
                </div>
